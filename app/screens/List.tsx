...
const List = ({navigation} : any) => {
    ...
    useEffect(() =>{
        const todoRef = collection(FIRESTORE_BD, 'todos');
    ...
    const addTodo = async() =>{
        const doc = await addDoc(collection(FIRESTORE_BD, 'todos'), {title: todo, done: false});
        setTodo('')
    };

    const renderTodo = ({item} : any) => {

        const ref = doc(FIRESTORE_BD, `todos/${item.id}`);
  ...
  return (
    <View style={styles.container}>
      ...
      <Image
      source={require('../../assets/emojioculos.png')}
      style={styles.emoji}/>
    </View>
  )
}
